...
export default function FriendGroupDisplay({friendGroup, friends}: FriendGroupDisplayProps) {
    ...
    const sortByAvailability = (friend1: Friend, friend2: Friend) => {
        ...
        if (friend1Index === friend2Index) {
            return sortAlphabetically(friend1, friend2);
        }
            ...
            <div className={styles.groupHeaderContainer}>
                ...
                <div className={styles.longFlex}>
                    ...
                    <div className={styles.arrowContainer} onClick={toggleCollapsed}>
                        <svg className={Globals.applyMultipleStyles(styles.arrow, collapsed ? styles.collapsedArrow : '')} viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"><path d="M768 903.232l-50.432 56.768L256 512l461.568-448 50.432 56.768L364.928 512z" fill="#ffffff"/></svg>
                    </div>
                </div>
            </div>
            <div className={Globals.applyMultipleStyles(styles.friendsContainer, collapsed ? styles.collapsedFriends : '')}>
                {
                    friends?.sort(
                        (friend1, friend2) => sortByAvailability(friend1, friend2)
                    ).map((friend) => {
                        return <SingleFriendDisplay friend={friend} key={friend.puuid}/>;
                    })
                }
            </div>
        </div>
