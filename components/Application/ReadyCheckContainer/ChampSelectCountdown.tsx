...
export default function ChampSelectCountdown({accepted, declined}: ChampSelectCountdownProps) {
    ...
    useEffect(() => {
        if (declined) setColor(colorDecline);
    }, [declined]);
    ...
    return (
        <>
            <svg viewBox="-125 -125 250 250" xmlns="http://www.w3.org/2000/svg" style={{transform: 'rotate(45deg)'}} fill="transparent">
                <circle r="100" cx="0" cy="0" fill={'#181a1b'} stroke={color} strokeWidth="16px" strokeDasharray={'628.32px'} strokeDashoffset={(accepted||declined)?(pixelPerPercent * initialProgressPercent):('-628.32px')}>
                    {(!(accepted || declined))? (
