...
export default function GamemodeElement(data: GamemodeElementProps) {
    ...
    const getActiveVideoPath = (gamemode: string, mapId: number) => {
        return Globals.PROXY_STATIC_PREFIX + '/' + Globals.getSpecificMapAsset(
            mapAssets,
            mapId,
            gamemode,
    ...
    const getHoveredImagePath = (gamemode: string, mapId: number) => {
        return Globals.PROXY_STATIC_PREFIX + '/' + Globals.getSpecificMapAsset(
            mapAssets,
            mapId,
            gamemode,
    ...
    const getNormalImagePath = (gamemode: string, mapId: number) => {
        return Globals.PROXY_STATIC_PREFIX + '/' + Globals.getSpecificMapAsset(
            mapAssets,
            mapId,
            gamemode,
                    ...
                    data.active ?
                        <div className={styles.queueSelection}>
                            {data.queues
                                ...
                                .map((queue) => {
                                    return (
                                        <div className={Globals.applyMultipleStyles(styles.queueButton,data.activeQueueId === queue.id ? styles.active: '' )} key={queue.id} onClick={() => data.setActiveQueueId(queue.id)}>
                                            {queue.description}
                                        </div>
                                    );
                                })}
